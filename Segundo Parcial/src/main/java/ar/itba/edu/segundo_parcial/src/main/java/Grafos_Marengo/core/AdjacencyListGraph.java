package Grafos_Marengo.core;

import java.lang.reflect.Method;
import java.util.*;

import Grafos_Marengo.core_service.GraphService;
import Parcial2023_2C.EmptyEdgeProp;
...
abstract public class AdjacencyListGraph<V, E> implements GraphService<V, E> {
	...
	public void printBFS(V startNode) {
		...
		while( ! theQueue.isEmpty()) {
			...
			if (!visited.contains(current)) {
				...
				Collection<InternalEdge> vecinos = getAdjacencyList().get(current);
	...
	public boolean hasCycle() {
		...
		for (V vertex : adjacencyList.keySet()) {
			if (!visited.contains(vertex)) {
				if (hasCyclesRec(vertex, visited, null))
					return true;
			}
		}
	...
	private boolean hasCyclesRec(V vertex, Set<V> visited, V parent) {
		visited.add(vertex);
		for (InternalEdge internalEdge : adjacencyList.get(vertex)) {
			V adjacentVertex = internalEdge.target;

			if (!visited.contains(adjacentVertex)) {
				if (hasCyclesRec(adjacentVertex, visited, vertex))
					return true;
