	...
	public void addEdge(V aVertex, V otherVertex, E theEdge) {
		...
		for (InternalEdge internalEdge : adjListSrc) {
			...
			if (internalEdge.target.equals(otherVertex))
				throw new IllegalArgumentException(String.format("simpleOrDefault graph cannot contain several edges with same source %s and target %s ", aVertex, otherVertex));
				//throw new IllegalArgumentException(String.format(Messages.getString("addEdgeSimpleOrDefaultError"), aVertex, otherVertex));
				/*
				* @Todo no se porque el otro throw no anda.
				 */
		}
