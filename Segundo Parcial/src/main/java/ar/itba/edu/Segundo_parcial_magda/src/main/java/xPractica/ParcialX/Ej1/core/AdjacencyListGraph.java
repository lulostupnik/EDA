...
abstract public class AdjacencyListGraph<V, E> implements GraphService<V, E> {
    ...
    public int inDegreeWithWeigth(V aVertex) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        if (aVertex == null)
        ...
        for (V vertex : adjacencyList.keySet()) {
            for (InternalEdge internalEdge : adjacencyList.get(vertex)) {
                if (internalEdge.target.equals(aVertex)) {
                    Method fn = internalEdge.edge.getClass().getMethod("getWeight"); // obtengo el peso de cada edge
                    count += (int) fn.invoke(internalEdge.edge); // sumo los pesos
                }
            }
        }
        return count;
    ...
    public void addByTargetVertex(GraphService<V, E> graph, V target) {
        if (!graph.getVertices().contains(target)) // si el vertice no esta en el subgrafo
            graph.addVertex(target); // lo agrego
        for (V vert : getVertices()) {
            if (!vert.equals(target)) {
                for (InternalEdge edges : adjacencyList.get(vert)) {
                    if (edges.target.equals(target))
                        graph.addEdge(vert, target, edges.edge);
                }
            }
        }
    }

    public GraphService<V, E> popularSubgraph(int n) throws NoSuchMethodException, InvocationTargetException, IllegalAccessException {
        if (!isDirected || !isWeighted)
            throw new RuntimeException("This method is only for directed weighted graphs");
        GraphService<V, E> subgraph = GraphFactory.create(Multiplicity.SIMPLE, EdgeMode.DIRECTED, SelfLoop.NO, Weight.YES, Storage.SPARSE);

        for (V vertex : getVertices()) { // recorro todos los vertices
            if (inDegreeWithWeigth(vertex) >= n)
                addByTargetVertex(subgraph, vertex);
        }
        return subgraph;
    }
    ...
    public boolean removeVertex(V aVertex) {
        ...
        if (!isDirected) {
            for (InternalEdge internalEdge : adjacencyList.get(aVertex))
                adjacencyList.get(internalEdge.target).removeIf(edge -> edge.target.equals(aVertex));
        } else {
            for (V vertex : adjacencyList.keySet())
                adjacencyList.get(vertex).removeIf(edge -> edge.target.equals(aVertex));
        }
    ...
    public DijkstraPath<V, E> dijsktra(V source) {
        ...
        while (!pq.isEmpty()) {
            ...
            Collection<InternalEdge> adjList = getAdjacencyList().get(current.vertex);
    ...
    public static void main(String[] args) {
        ...
        */
        ...
        GraphService<String, WeightedEdge> g2 = GraphFactory.create(Multiplicity.SIMPLE, EdgeMode.DIRECTED, SelfLoop.NO, Weight.YES, Storage.SPARSE);
