...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    private int size = 0;
    private REMOVE_PREFERENCE preference = REMOVE_PREFERENCE.LEFT_BROTHER_HIGHEST_LEXI;
    ...
    public BTree(int order, REMOVE_PREFERENCE preference) {
        this.minKeySize = order;
        this.minChildrenSize = minKeySize + 1;
        this.maxKeySize = 2 * minKeySize;
        this.maxChildrenSize = maxKeySize + 1;
    ...
    private T remove(T value, Node<T> node) {
        ...
        if (node.numberOfChildren() == 0) {
            ...
            if(preference == REMOVE_PREFERENCE.LEFT_BROTHER_HIGHEST_LEXI){
    ...
    public java.util.Collection<T> toCollection() {
        return (new JavaCompatibleBTree2<T>(this));
    }
    ...
    private static class Node<T extends Comparable<T>> {
        ...
        private Comparator<Node<T>> comparator = new Comparator<BTree.Node<T>>() {
            @Override
            public int compare(BTree.Node<T> arg0, BTree.Node<T> arg1) {
    ...
    public static class JavaCompatibleBTree2<T extends Comparable<T>> extends java.util.AbstractCollection<T> {

        private BTree<T> tree = null;

        public JavaCompatibleBTree2(BTree<T> tree) {
            this.tree = tree;
        }
        ...
        public java.util.Iterator<T> iterator() {
            return (new BTree2Iterator<T>(this.tree));
        }

        private static class BTree2Iterator<C extends Comparable<C>> implements java.util.Iterator<C> {
            ...
            protected BTree2Iterator(BTree<C> tree) {
                this.tree = tree;
