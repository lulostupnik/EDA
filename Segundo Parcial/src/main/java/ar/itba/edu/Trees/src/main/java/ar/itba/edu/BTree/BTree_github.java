...
//cambiar linea 200 por el otro lexicografico
/**
...
public class BTree_github<T extends Comparable<T>> implements ITree<T> {
...
//Implementacion Agregada
    ...
    public T remove2(T value) {
        throw new IllegalArgumentException("Not implemented");
//        T removed = null;
//        Node<T> node = this.getNode(value);
//        removed = removeLesser(value,node);
//        return removed;
    }
   ...
   /* private T removeLesserValue(Node<T> node) {
    ...
    private T removeLesser(T value, Node<T> node) {
        if (node == null) return null;
        ...
        if (node.numberOfChildren() == 0) {
            ...
            // internal node
            Node<T> greatest = node.getChild(node.numberOfChildren() - 1);
...
*/
    ...
    private static class TreePrinter {
        ...
        public static <T extends Comparable<T>> String getString(BTree_github<T> tree) {
    ...
    public static class JavaCompatibleBTree<T extends Comparable<T>> extends java.util.AbstractCollection<T> {
        ...
        private BTree_github<T> tree = null;

        public JavaCompatibleBTree(BTree_github<T> tree) {
            this.tree = tree;
        }
        ...
        private static class BTreeIterator<C extends Comparable<C>> implements java.util.Iterator<C> {
            ...
            private BTree_github<C> tree = null;
            private BTree_github.Node<C> lastNode = null;
            private C lastValue = null;
            private int index = 0;
            private Deque<BTree_github.Node<C>> toVisit = new ArrayDeque<BTree_github.Node<C>>();

            protected BTreeIterator(BTree_github<C> tree) {
                this.tree = tree;
            ...
            public C next() {
                ...
                while (toVisit.size()>0) {
                    ...
                    BTree_github.Node<C> n = toVisit.pop();
