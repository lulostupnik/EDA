package ar.edu.itba.eda.Lucene.src.main.java;
...
public class IndexBuilder {
	...
	private static void createIndex() throws IOException 	{
		...
		List<String> fileIterator = core.Utils.listFilesRelativePath( "docs", Arrays.asList("txt") );
	...
	private static IndexWriter initIndexWriter() throws IOException {
		...
		Directory indexDir = FSDirectory.open( Paths.get(core.Utils.getPrefixDir() + "/index/"));
		
		IndexWriterConfig indexConfig = new IndexWriterConfig(new StandardAnalyzer());
		
		// create or overwrites an existing one
		indexConfig.setOpenMode(IndexWriterConfig.OpenMode.CREATE);  // other options are available
